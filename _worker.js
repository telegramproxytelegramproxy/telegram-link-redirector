export default {
  async fetch(request) {
    ...
    // Danh sách path đặc biệt cần xử lý riêng
    const specialPaths = {
    ...
    for (const [pathPrefix, config] of Object.entries(specialPaths)) {
      if (cleanPath.startsWith(pathPrefix)) {
        ...
        originalPath = config.path + cleanPath.slice(pathPrefix.length);
        break;
      }
    }
    
    // Nếu không phải path đặc biệt, xử lý theo subdomain
    if (targetDomain === 'telegram.org' && cleanPath.split('/').length > 1) {
      const potentialSubdomain = cleanPath.split('/')[1];
      if (potentialSubdomain) {
        targetDomain = `${potentialSubdomain}.telegram.org`;
        originalPath = cleanPath.slice(potentialSubdomain.length + 1) || '/';
      }
    }
    
    // Xử lý t.me
    if (url.hostname === baseDomain && cleanPath.startsWith('/t/')) {
      targetDomain = 't.me';
      originalPath = cleanPath.slice(2); // Giữ lại '/' sau 't'
    }
    
    // Xây dựng target URL (đảm bảo không có dấu / liên tiếp)
    const targetUrl = `https://${targetDomain}${originalPath.replace(/\/+/g, '/')}${url.search}`;
